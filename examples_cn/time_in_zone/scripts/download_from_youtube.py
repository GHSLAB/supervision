...
def main(
    ...
    """
    通过提供 URL 来下载指定的 YouTube 视频。

    Args:
        url: 待下载 YouTube 视频的完整 URL。
        output_path: 视频保存的目标目录。
        file_name: 保存的视频文件名。
    """
    ...
    try:
        ...
        print(f"下载失败：{err}", file=sys.stderr)
        sys.exit(1)

    final_name = file_name if file_name else "视频标题"
    final_path = output_path if output_path else "当前目录"
    print(f"下载完成！视频已保存为 '{final_name}'，位置：'{final_path}'。")
