...
def detect(
    ...
    """
    使用 YOLO 模型在帧中检测目标，并按类别 ID 和置信度进行过滤。

    Args:
        frame (np.ndarray): 待处理的帧，应为 NumPy 数组。
        model (YOLO): 用于处理帧的 YOLO 模型。
        confidence_threshold (float): 用于过滤检测结果的置信度阈值，默认为 0.5。
        iou_threshold (float): 非极大值抑制的 IOU 阈值。

    Returns:
        sv.Detections: 使用 YOLO 模型处理帧后过滤得到的检测结果。

    Note:
        该函数针对 YOLO 模型定制，假设使用类别 ID 0 进行过滤。
    """
    results = model(
...
def main(
    ...
    """
    使用 YOLO 和 Supervision 在区域中统计人数。
